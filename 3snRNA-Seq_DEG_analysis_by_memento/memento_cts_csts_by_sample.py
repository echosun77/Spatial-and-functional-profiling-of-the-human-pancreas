...
for fea in ['cell_type', 'cell_subtype']:
    ...
    for i, cell_type in enumerate(fea_list):
        ...
        for group in ['ND - Obese', 'T2D']:
            ...
            print("Label columns used:", adata_tmp.uns['memento']['label_columns'], flush=True)
            print("Groups created:", adata_tmp.uns['memento']['groups'], flush=True)
            ...
            if adata_tmp.shape[1] == 0:
                ...
                result_1d.to_csv(
                        f"../tables/by_sample/memento_ht_1d_moments_{group}_{fea}_{cell_type.replace(' ', '')}_by_sample.csv",
                    sep=',', index=False
                )
