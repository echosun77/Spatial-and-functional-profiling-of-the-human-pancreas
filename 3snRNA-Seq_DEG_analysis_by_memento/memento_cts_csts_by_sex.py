...
fea_lists = {
    'cell_type': ['Acinar', 'Endothelial', 'Ductal', 'Endocrine'],
    'cell_subtype': ['α', 'β', 'γ', 'δ', 'Acinar_0', 'Acinar_1', 'Acinar_2', 'Ductal_0', 'Ductal_1', 'Ductal_2']
}
    ...
    for i, cell_type in enumerate(fea_list):
        ...
        for group in ['ND - Obese', 'T2D']:
            ...
            adata_tmp = adata[adata.obs['group'].isin(['ND - Lean', group]) &
                              ...
                              adata.obs['Sex'].isin(samples)].copy()
            ...
            if adata_tmp.shape[1] == 0:
                ...
                result_1d.to_csv(
                    f"../tables/by_sex/memento_ht_1d_moments_{group}_{fea}_{cell_type.replace(' ', '')}_by_sex.csv",
                    sep=',', index=False
                )
