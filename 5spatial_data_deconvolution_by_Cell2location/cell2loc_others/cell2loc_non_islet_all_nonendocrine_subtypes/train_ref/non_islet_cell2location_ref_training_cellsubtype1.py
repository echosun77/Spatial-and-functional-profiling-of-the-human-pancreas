...
data_folder = '../../data/'
res_folder = '../../results/cell2loc_non_islet/cst_ref'
...
ENDO_TYPES = ['Signaling_acinar', 'Intermediate_acinar',
       'High_enzyme_acinar', 'Basal_ductal', 'Inflam_ductal_1',
       'Inflam_ductal_2', 'MUC5B+_ductal', 'Arterial_ECs', 'Venous_ECs',
       'Capillary_ECs', 'Lymphatic_ECs', 'Pancreatic_resident_macrophages',
       'CD27-_IgA+_plasmablasts', 'Pancreatic_persistent_memory_like_T_cells',
       'Mast_cells', 'Activated_stellates', 'Quiescent_stellates', 'Schwann']   # (= PP/γ)

REF_CTYPE_COL = "cell_subtype2"   # <-- change if your ref uses a different column
...
parse_file = os.path.join(res_folder, 'sn_non_islet_cst_ref.h5ad')
parse_ref.write(parse_file)

plt.figure()
rm.plot_history(10)
plt.savefig(os.path.join(res_folder, 'ref_training_detection_cst.png'), dpi=300, bbox_inches='tight')
...
plt.savefig(os.path.join(res_folder, 'ref_training_QC_cst.png'), dpi=300, bbox_inches='tight')
