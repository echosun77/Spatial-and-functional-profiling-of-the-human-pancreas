...
res_folder = f'../../../results/cell2loc_others/'
data_folder = '../../../data/'

os.makedirs(data_folder, exist_ok=True)
os.makedirs(res_folder, exist_ok=True)
os.makedirs(os.path.join(res_folder, f'{ct.lower()}/{sample_type}'), exist_ok=True)

print("load spatial and reference datasets")
spatial= sc.read_h5ad(os.path.join(
    data_folder,
    'YK_raw_spatial_islet_extra_islet_c2l_annot_hq.h5ad'))

high_quality_samples = ['C7', 'C13', 'C15', 'C19', 'P5', 'U2', 'U6', 
                        'U11', 'V26', 'V31', 'V33', 'V34', 'V35', 'V36']
print("only work train on high quality samples")
spatial_tmp = spatial[
        spatial.obs['sample'].isin(high_quality_samples) &
        spatial.obs['max_pred_celltype'].isin([ct])
        ].copy()

print("remove mitochondria-encoded (MT) genes from the spatial dataset")
spatial_tmp.var['MT_gene'] = [gene.startswith('MT-') for gene in spatial_tmp.var_names]
spatial_tmp.obsm['MT'] = spatial_tmp[:, spatial_tmp.var['MT_gene'].values].X.toarray()
spatial_tmp = spatial_tmp[:, ~spatial_tmp.var['MT_gene'].values]
spat = spatial_tmp.copy()

from cell2location.models import RegressionModel, Cell2location
## ===== 1) Import the saved model trained on ref data
print("import the model from ref data")
parse_file = os.path.join(res_folder, f'cst_ref/{ct.lower()}/sn_{ct.lower()}_cst_ref.h5ad')
parse_ref = sc.read_h5ad(parse_file)
rm = RegressionModel.load(os.path.join(res_folder, f'cst_ref/{ct.lower()}'), parse_ref)
...
c2l = Cell2location(
    ...
    # islet cells are denser to use 1.5, exocrine cells suggest to use 1.2
...
max_total_epochs = 2500     # hard ceiling so we never run forever
...
plt.savefig(
        os.path.join(
            res_folder,
            f'{ct.lower()}/{sample_type}/detection_{ct.lower()}_{sample_type}.png'), 
        dpi=300)
...
# Save model
c2l.save(os.path.join(res_folder, f'{ct.lower()}/{sample_type}'), overwrite=True)
spatial_file = os.path.join(
        res_folder,
        f"{ct.lower()}/{sample_type}/sp_c2l_{ct.lower()}_{sample_type}.h5ad"
        )

spat.obs["islet_label"] = spat.obs["islet_label"].astype("object")                                   
spat.obs["islet_label_smooth"] = spat.obs["islet_label_smooth"].astype("object")
spat.obs["islet_label_sharpen"] = spat.obs["islet_label_sharpen"].astype("object")
...
plt.savefig(
    os.path.join(
        res_folder,
        f'{ct.lower()}/{sample_type}/c2l_QC_{ct.lower()}_{sample_type}.png'), 
    dpi=300)
