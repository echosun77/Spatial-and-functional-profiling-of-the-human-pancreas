...
data_folder = '../../../data/'
res_folder = f'../../../results/cell2loc_others/cst_ref/{ct.lower()}/'
...
parse = parse[(parse.obs['Doublets'] == 'no') & (parse.obs['cell_type'] == ct)]
...
rm.train(max_epochs=2500) # you may increase max epochs by checking rm.plot_history() to reach plat
...
parse_file = os.path.join(res_folder, f'sn_{ct.lower()}_cst_ref.h5ad')
parse_ref.write(parse_file)

plt.figure()
rm.plot_history(10)
plt.savefig(os.path.join(res_folder, f'ref_training_detection_{ct.lower()}_cst.png'), dpi=300, bbox_inches='tight')
...
plt.savefig(os.path.join(res_folder, f'ref_training_QC_{ct.lower()}_cst.png'), dpi=300, bbox_inches='tight')
