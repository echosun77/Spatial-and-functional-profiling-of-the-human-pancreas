...
data_folder = '../../../data/'
res_folder = '../../../results/cell2loc_others/cst_ref/ductal/'
...
parse = parse[(parse.obs['Doublets'] == 'no') & (parse.obs['cell_type'] ==
    'Ductal')]
...
rm.train(max_epochs=250) # you may increase max epochs by checking rm.plot_history() to reach plat
...
parse_file = os.path.join(res_folder, 'sn_ductal_cst_ref.h5ad')
parse_ref.write(parse_file)

plt.figure()
rm.plot_history(10)
plt.savefig(os.path.join(res_folder, 'ref_training_detection_ductal_cst.png'), dpi=300, bbox_inches='tight')
...
plt.savefig(os.path.join(res_folder, 'ref_training_QC_ductal_cst.png'), dpi=300, bbox_inches='tight')
