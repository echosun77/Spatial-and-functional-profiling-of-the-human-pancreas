...
fig_folder = '../../figures/cell2loc_extra_islets/'
res_folder = f'../../results/cell2loc_extra_islets/HQ_samples/'
data_folder = '../../data/'
...
spatial_islet = sc.read_h5ad(os.path.join(data_folder, 'spatial_extra_endocrine.h5ad'))
parse_islet = sc.read_h5ad(os.path.join(data_folder, 'cell2loc_islets/parse_islet.h5ad'))

high_quality_samples = ['C7', 'C13', 'C15', 'C19', 'P5', 'U2', 'U6', 
                        'U11', 'V26', 'V31', 'V33', 'V34', 'V35', 'V36']
print("only work train on high quality samples")
spatial_islet = spatial_islet[spatial_islet.obs['sample'].isin(high_quality_samples)].copy()
...
parse_file = f"{os.path.join(data_folder, 'cell2loc_islets/ref/sn_islet_ref.h5ad')}"
parse_ref = sc.read_h5ad(parse_file)
rm = RegressionModel.load(f"{os.path.join(data_folder, 'cell2loc_islets/ref')}", parse_ref)
...
plt.figure()
c2l.plot_history(10)
plt.savefig(f'{res_folder}/detection_alpha_20_HQ.png', dpi=300)
...
spatial_file = f"{res_folder}/spatial_extra_islet_after_c2l_HQ.h5ad"
spat.write(spatial_file)

plt.figure()
c2l.plot_QC()
plt.savefig(f'{res_folder}/c2l_QC_HQ.png', dpi=300)
